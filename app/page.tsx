...
import { cn, logErrorToSupabase } from "@/lib/utils";
...
const popularLanguages = ["English", "Spanish", "French", "German", "Mandarin", "Portuguese"];
...
const formSchema = z.object({
    ...
    department: z.enum(["development", "implementation", "sales", "marketing"], {
...
export default function OnboardingForm() {
    ...
    const filteredTimeZones = useMemo(
        () => tzOptions.filter((item) => item.label.toLowerCase().includes(tzQuery.toLowerCase())),
        [tzOptions, tzQuery]
    );
    ...
    const filteredCurrencies = useMemo(
        () => currencyList.filter((item) => item.code.toLowerCase().includes(currencyQuery.toLowerCase())),
        [currencyQuery]
    );
    ...
    const filteredLanguages = useMemo(
        () => languagesList.filter((item) => item.label.toLowerCase().includes(langQuery.toLowerCase())),
        [langQuery]
    );
    ...
    async function onSubmit(values: FormValues) {
        ...
        try {
            ...
            await logErrorToSupabase(err, "onSubmit");
            toast.error("Failed to submit the form. Please try again.");
        } finally {
            setLoading(false);
        }
        ...
        <div className="p-8 overflow-hidden">
            <Form {...form}>
                ...
                >
                    ...
                    {/* ────── Submit ────── */}
                    <Button type="submit" disabled={loading} className="mt-6 ml-auto bg-[#002B89]">
