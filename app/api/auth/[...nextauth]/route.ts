...
async function refreshAccessToken(token: Token): Promise<Token> {
    try {
        ...
        await logErrorToSupabase(error, "refreshAccessToken");
        return {
            ...token,
...
const authOptions: NextAuthOptions = {
    ...
    callbacks: {
        async signIn({ user }) {
            ...
            console.log(email);
            if (email.endsWith(process.env.NEXT_PUBLIC_DOMAIN!)) {
                return true;
            }
            return false;
        },
        async jwt({ token, account, user }) {
            ...
            if (
                (token as unknown as Token).accessTokenExpires &&
                Date.now() < (token as unknown as Token).accessTokenExpires
            ) {
                return token;
            }
